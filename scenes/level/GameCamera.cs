...
public partial class GameCamera : Camera2D
{
	...
	private const float NOISE_FREQUENCY_MULTIPILER = 100f;
	private const float SHAKE_DECAY = 1;
	...
	private float currentShakePercentage = 1;

	// Export funkcji prywatnych takich jak wstrząs na ekranie i wartość wstrząsu
	...
	// Wstrząs o wartości ustawionej na 1
	...
	private void ApplyCameraShake(double delta)
	{
		...
		{
			noiseSample.X += NOISE_SAMPLE_GROWTH * NOISE_FREQUENCY_MULTIPILER * (float)delta;
			noiseSample.Y += NOISE_SAMPLE_GROWTH * NOISE_FREQUENCY_MULTIPILER * (float)delta;
